 ...
 * locate a suitable st
 * ack from amongst those loaded at
...
//  ToDo  ClassLoader callerCL = ClassLoader.getSystemClassLoader();
    ...
    for (StackInfo di : stacks) {
      ...
      if (getCallerClass(di.stackClassName) != di.stackClass) {
  ...
  public static synchronized void deregisterStack(Stack stack) throws InternalException {
    ...
    if (stackInfo == null || getCallerClass(stackInfo.stackClassName) != stackInfo.stackClass) {
...
//  }
  ...
  private static Class getCallerClass(String stackClassName) {
  Class callerC;
  try {
    callerC = Class.forName(stackClassName);
  } catch (Exception ex) {
    callerC = null;
  }
  return callerC;
